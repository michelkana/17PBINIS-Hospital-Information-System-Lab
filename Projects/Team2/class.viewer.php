<?php
	...
	$html="<html>
					<head>
						<title>Nastavení omezení pacienta </title>
						<meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"> 
					</head>
					<body>
						<h3>Nastavení omezení pacienta</h3>
	...
	$html = "
				<form method=\"post\" action=\"class.viewer.php?command_id=22\">
							<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
                                ...
                                <tr>
									<td>Opakovat dalsi dny? (1-7, 0 pro NE))</td>
									<td><input type=\"text\" name=\"r_day\" size=\"5\"></td>
								</tr>
                                <tr>
									<td>Opakovat dalsi tydny? (1-4, 0 pro NE)</td>
									<td><input type=\"text\" name=\"r_week\" size=\"5\"></td>
								</tr>
                                <tr>
									<td>Opakovat dalsi mesice? (1-12, 0 pro NE)</td>
									<td><input type=\"text\" name=\"r_month\" size=\"5\"></td>
								</tr>
								<tr>
		...
		$html = "
				<form method=\"post\" action=\"class.viewer.php?command_id=25\">
							<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
								...
								<tr>
									<td>Příjmení</td>
									<td><input type=\"text\" name=\"last\" size=\"15\"></td>
								</tr>
								<tr>
									<td>E-mail</td>
...
///////////////////////////////////////////////////////////
	...
	elseif($command_id == 16){
		...
		$message="Pacient byl autorizován a byl mu odeslán email s přístupovými informacemi";
		$gui->display_homepage($user_name, $user_department_name, $message);
	...
	elseif($command_id == 22){
        ...
        $message = "Dostupnost byla pridana";
        if ($r_day>1 || $r_week>1 || $r_month>1) {
            $distrib->request_repeat($current_user_id,$day,$month,$year,$r_day,$r_week,$r_month,$time_in,$time_out);
            $message = $message . " a opakovany byly take pridany";
        }
		$gui->display_homepage($user_name, $user_department_name, $message);
	}
